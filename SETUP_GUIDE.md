...
| Claude Code or GitHub Copilot CLI   | Latest  | Required for the Claude Code plugin path; `claude --version` or `copilot --version`            |
...
* **VS Code extension**: Open Copilot Chat and type `@`. You should see the Copilot Studio agents (Manage, Author, Test, Advisor) in the list.
...
* [ ] Agent cloned with `@copilot-studio:copilot-studio-manage clone` or the VS Code Extension
